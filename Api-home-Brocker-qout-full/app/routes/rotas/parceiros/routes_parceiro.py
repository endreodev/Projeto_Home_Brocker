...
from app.functions.validacao_empresa_filial import acessoEmpresa, acessoFilial, inFiliais
...
def init_routes_parceiro(app):
    ...
    def create_parceiro():
        ...
        if not empresa.ativo:
            return jsonify({'message': 'Empresa desativada.'}), 404
        
        quantidade = db.session.query(Parceiro).filter(Parceiro.empresa_id == empresa_id).count()
        if quantidade >= empresa.limiteparc:
            return jsonify({'message': 'O Plano da sua só permite 5 parceiros! Entre em contato para almentar seu plano!'}), 404
    ...
    def get_parceiro():
        ...
        parceiros = db.session.query(
            ...
            Grupo, Parceiro.grupo_id == Grupo.id
        ).filter(
            Parceiro.empresa_id == empresa_id  # Filtra parceiros pelo empresa_id
        ).all()
    ...
    def get_parceiro_id(id):
        ...
        # claims = get_jwt()
        # if not acessoFilial(claims , parceiro):
